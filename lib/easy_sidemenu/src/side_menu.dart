...
import "package:u/easy_sidemenu/src/models/side_menu_item_type.dart";
...
import "package:u/easy_sidemenu/src/side_menu_expansion_item.dart";
import "package:u/easy_sidemenu/src/side_menu_expansion_item_with_global.dart";
import "package:u/easy_sidemenu/src/side_menu_hamburger_mode.dart";
import "package:u/easy_sidemenu/src/side_menu_item.dart";
import "package:u/easy_sidemenu/src/side_menu_item_with_global.dart";
import "package:u/easy_sidemenu/src/side_menu_style.dart";
import "package:u/easy_sidemenu/src/side_menu_toggle.dart";
...
class _SideMenuState extends State<SideMenu> {
  ...
  void _initializeSideMenu() {
    ...
    if (global.expansionStateList.length != sideMenuExpansionItemCount) {
      ...
      for (final SideMenuItemType item in widget.items) {
    ...
    sidemenuitems.items = widget.items.map((SideMenuItemType data) {
      if (data is SideMenuItem) {
        ...
        return SideMenuExpansionItemWithGlobal(
          ...
          children: data.children
              .map(
                (SideMenuItem childData) => SideMenuItemWithGlobal(
                  global: global,
  ...
  void _updateWidth() {
    final double newWidth = _calculateWidth(global.displayModeState.value, context);
  ...
  void didUpdateWidget(covariant SideMenu oldWidget) {
    ...
    if (widget.controller != oldWidget.controller || widget.items != oldWidget.items) {
      _initializeSideMenu();
    }
  ...
  double _calculateOpenWidth() {
    ...
    _notifyParent();
    Future<void>.delayed(_toggleDuration(), () {
      if (mounted) {
  ...
  Duration _toggleDuration() => widget.displayModeToggleDuration ?? const Duration(milliseconds: 350);
  ...
  Widget build(BuildContext context) => ChangeNotifierProvider<Global>.value(
    ...
    child: Builder(
      builder: (BuildContext context) {
        ...
        return ((global.style.showHamburger) && (_hamburgerMode == SideMenuHamburgerMode.close))
                    ...
                    if (global.style.displayMode != SideMenuDisplayMode.auto && showToggle)
                      Padding(
                        padding: EdgeInsets.symmetric(
                          horizontal: global.displayModeState.value == SideMenuDisplayMode.open ? 0 : 4,
                        ),
                        child: Row(
