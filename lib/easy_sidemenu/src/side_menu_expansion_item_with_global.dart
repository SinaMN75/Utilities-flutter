...
class _SideMenuExpansionState extends State<SideMenuExpansionItemWithGlobal> {
  ...
  Widget build(BuildContext context) => Consumer<Global>(
    builder: (BuildContext context, Global global, Widget? child) => ValueListenableBuilder(
      valueListenable: global.displayModeState,
      builder: (BuildContext context, Object? value, Widget? child) => ListTileTheme(
        contentPadding: EdgeInsets.symmetric(
          horizontal: value == SideMenuDisplayMode.compact ? global.style.itemInnerSpacing : global.style.itemInnerSpacing + 5,
        ),
        ...
        child: ExpansionTile(
          ...
          trailing: Icon(
            ...
            color: isExpanded ? global.style.arrowOpen : global.style.arrowCollapse,
          ),
          initiallyExpanded: global.expansionStateList[widget.index],
          title: (value == SideMenuDisplayMode.open)
              ? Text(
                  widget.title ?? "",
                  style: global.expansionStateList[widget.index] ? const TextStyle(fontSize: 17, color: Colors.black).merge(global.style.selectedTitleTextStyleExpandable ?? global.style.selectedTitleTextStyle) : const TextStyle(fontSize: 17, color: Colors.black54).merge(global.style.unselectedTitleTextStyleExpandable ?? global.style.unselectedTitleTextStyle),
                )
              : const Text(""),
