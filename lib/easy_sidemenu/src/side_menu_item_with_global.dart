...
import "package:u/easy_sidemenu/src/global/global.dart";
...
import "package:u/easy_sidemenu/src/side_menu_expansion_item_with_global.dart";
...
class _SideMenuItemState extends State<SideMenuItemWithGlobal> {
  ...
  void initState() {
    super.initState();
    _nonNullableWrap(WidgetsBinding.instance)!.addPostFrameCallback((Duration timeStamp) {
  ...
  int _getIndexOfCurrentSideMenuItemWidget() {
    int index = 0;
    for (final SideMenuItemWithGlobalBase itemBase in widget.global.items) {
      ...
      if (itemBase is SideMenuItemWithGlobal) {
        ...
        for (final SideMenuItemWithGlobal childItem in itemBase.children) {
  ...
  Widget _generateIcon(Icon? mainIcon, Widget? iconWidget) {
    ...
    final Color iconColor = _isCurrentSideMenuItemSelected() ? widget.global.style.selectedIconColor ?? Colors.black : widget.global.style.unselectedIconColor ?? Colors.black54;
  ...
  bool _isCurrentSideMenuItemSelected() => _getIndexOfCurrentSideMenuItemWidget() == currentPage;

  @override
  Widget build(BuildContext context) => Consumer<Global>(
    builder: (BuildContext context, Global global, Widget? child) {
      if (widget.builder == null) {
        return InkWell(
          onTap: () => widget.onTap?.call(_getIndexOfCurrentSideMenuItemWidget(), global.controller),
          onHover: (bool value) {
            safeSetState(() {
              isHovered = value;
            });
          ...
          child: Padding(
            ...
            child: Container(
              ...
              child: ValueListenableBuilder(
                ...
                builder: (BuildContext context, SideMenuDisplayMode value, Widget? child) => Tooltip(
                  message: (value == SideMenuDisplayMode.compact && global.style.showTooltip) ? widget.tooltipContent ?? widget.title ?? "" : "",
                  child: Padding(
                    padding: EdgeInsets.symmetric(vertical: value == SideMenuDisplayMode.compact ? global.style.itemInnerSpacing : global.style.itemInnerSpacing),
                    child: Row(
                      children: <Widget>[
                        ...
                        if (value == SideMenuDisplayMode.open) ...<Widget>[
                          Expanded(
                            // Expanded will allow the text to take up all available space
                            child: Text(
                              widget.title ?? "",
                              overflow: TextOverflow.ellipsis, // Helps to handle long text
                              style: _getIndexOfCurrentSideMenuItemWidget() == currentPage.ceil() ? const TextStyle(fontSize: 17, color: Colors.black).merge(global.style.selectedTitleTextStyle) : const TextStyle(fontSize: 17, color: Colors.black54).merge(global.style.unselectedTitleTextStyle),
                            ),
                          ),
                          // const SizedBox.shrink(),
                          if (widget.trailing != null && global.showTrailing) ...<Widget>[
        ...
        return ValueListenableBuilder(
          valueListenable: global.displayModeState,
          builder: (BuildContext context, SideMenuDisplayMode value, Widget? child) => widget.builder!(context, value),
        );
      }
    },
  );
