...
class _CalendarRangePickerDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle saveButtonStyle = textTheme.bodyMedium!.apply(color: onConfirm != null ? headerForeground : headerDisabledForeground);
...
class _PInputDateRangePickerDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle? dateStyle = orientation == Orientation.landscape ? textTheme.titleMedium?.apply(color: dateColor) : textTheme.headlineMedium?.apply(color: dateColor);
    final String dateText = _formatDateRange(context, selectedStartDate, selectedEndDate, currentDate);
    final String semanticDateText = selectedStartDate != null && selectedEndDate != null ? '${selectedStartDate!.formatMediumDate()} – ${selectedEndDate!.formatMediumDate()}' : '';
