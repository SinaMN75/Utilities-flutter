...
class _SideMenuItemState extends State<SideMenuItemWithGlobal> {
  ...
  int _getIndexOfCurrentSideMenuItemWidget() {
    int index = 0;
    final int n = widget.global.items.length;
    for (int i = 0; i < n; i++) {
      if (widget.global.items[i] is SideMenuItemWithGlobal) {
        ...
        final int m = widget.global.items[i].children.length;
        for (int j = 0; j < m; j++) {
  ...
  Widget build(BuildContext context) {
    if (widget.builder == null) {
      return InkWell(
        ...
        child: Padding(
          ...
          child: Container(
            ...
            child: ValueListenableBuilder<SideMenuDisplayMode>(
              ...
              builder: (BuildContext context, SideMenuDisplayMode value, Widget? child) => Tooltip(
                message: (value == SideMenuDisplayMode.compact && widget.global.style.showTooltip) ? widget.tooltipContent ?? widget.title ?? "" : "",
                child: Padding(
                  padding: EdgeInsets.symmetric(vertical: value == SideMenuDisplayMode.compact ? widget.global.style.itemInnerSpacing : widget.global.style.itemInnerSpacing),
                  child: Row(
                    children: <Widget>[
                      ...
                      if (value == SideMenuDisplayMode.open) ...<Widget>[
                        Expanded(
                          ...
                          child: Text(
                            ...
                            style: _getIndexOfCurrentSideMenuItemWidget() == currentPage.ceil() ? const TextStyle(fontSize: 17, color: Colors.black).merge(widget.global.style.selectedTitleTextStyle) : const TextStyle(fontSize: 17, color: Colors.black54).merge(widget.global.style.unselectedTitleTextStyle),
                          ),
                        ),
                        // const SizedBox.shrink(),
                        if (widget.trailing != null && widget.global.showTrailing) ...<Widget>[
      ...
      return ValueListenableBuilder<SideMenuDisplayMode>(
        valueListenable: widget.global.displayModeState,
