...
|---------------------|-----------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `UApp`              | Device/platform/theme info        | `isAndroid/isIos/isWeb/isMacOs/isWindows/isLinux`, `isMobile/isDesktop`, `isDarkMode`, `isTablet()`, `size`, `theme`, `colorScheme`, `textTheme`, `locale()`, `updateLocale(l)`, `switchTheme(...)`, `isEmbedded`, `queryParameters`                                                                                                                                                                                                                                                                                                                                      |
| `UPwa`              | Web/PWA (in `utils/web/`)         | `isStandalone`, `isIphoneBrowser`, `isIosBrowser`, `isAndroidBrowser`, `isIosSafari`, `canPromptIosInstall`, `promptIosInstall(...)` (shows iOS "Add to Home Screen" guide). Web-only detection via conditional imports; safe no-ops on native.                                                                                                                                                                                                                                                                                                                           |
| `UWebMessage`       | Web postMessage (in `utils/web/`) | `listen(onMessage)` → disposer. Subscribes to browser `window` "message" events; no-op on native.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `UNavigator`        | Routing & dialogs                 | Nav: `push/off/offAll/offUntil`, `back/maybeBack/backUntil/backToRoot/backCount`, `fullScreenDialog`, `canPop/currentRouteName/context`. Dialogs: `dialog/animatedDialog/dialogResponsive`, `alert(...)`, `confirm(...)/confirmAsync(...)`, `inputDialog(...)`, `colorPicker(...)`. Pickers: `datePicker/dateRangePicker/timePicker`. Sheets: `bottomSheet/draggableSheet/actionSheet` (`UNavAction<T>`). Overlays: `showOverlay/dismissOverlay`. `RouteTransitions`: none/fade/fadeScale/slideFade/rightToLeft/leftToRight/upToDown/downToUp/scale/rotate/size/cupertino |
...
| `UToast`            | Snackbars / banners / toasts      | SnackBars: `snackBar(...)`, `success/error/warning/info(...)`, `clearSnackBars()`. Banners: `banner(...)`, `dismissBanner()`. Overlay toasts (animated, position-aware): `toast(...)`, `successToast/errorToast/warningToast/infoToast(...)`, `clearToast()`. Enums: `UToastType` (neutral/info/success/warning/error), `UToastPosition` (top/center/bottom)                                                                                                                                                                                                              |
| `UShare`            | Native share sheet                | `text(...)`, `link(...)`, `file(...)`, `files(...)`, `xFiles(...)`, `bytes(...)`, `widgetImage(controller:...)`, `wasShared(result)`. Wraps `share_plus`; returns `ShareResult`. (Prefer over `ULaunch.shareText/shareFile`.)                                                                                                                                                                                                                                                                                                                                             |
...
| `UCompress`         | Reduce file size (image/video)    | `Uint8List`-based, bytes in/out. `compress(bytes, extension, ...)` auto-routes; `image(bytes, quality, minWidth, minHeight, format)` cross-platform incl web; `video(bytes, extension, quality, includeAudio, webVideoCompressor)` native only, web uses hook; `typeOf(ext)`; `onVideoProgress(cb)`, `cancelVideo()`, `clearVideoCache()`. Native codecs, no FFmpeg (`u_compress.dart`)                                                                                                                                                                                   |
...
| `UScreenshot`       | Screenshot/recording guard        | `secure(enable)` (one-switch max protection), `disable()`, `enable()`, `toggle()`, `withBlur({blurRadius})`, `withColor({color})`, `withImage()`, `stream`, `startListening()`/`stopListening()`, `startRecordingListening()`/`stopRecordingListening()`, `onScreenshot(cb)`, `onScreenRecording(onStart, onStop)`, `removeCallbacks()`                                                                                                                                                                                                                                   |
